...
### 實戰 Demo：從做出一款遊戲到上線前全面體檢

> 以下是一段完整的開發流程：先用 Kiro 從零打造一款能按 Play 直接玩的 3D 射擊小遊戲，再對這款遊戲進行全方位的品質檢測與優化。把每一步的指令複製貼上到 Kiro 聊天框，你會親眼看到 Kiro 怎麼一步步幫你把事情做完。
...
你剛開了一個空專案，需要一個按 Play 就能跑的完整場景。

```
幫我建一個 3D 第一人稱射擊遊戲場景，包含：
- 地形（有起伏的草地）
- 方向光和環境光
- FPS 第一人稱控制器（WASD 移動 + 滑鼠視角）
- 幾個可當掩體的方塊和圓柱
- HUD Canvas 顯示準心和彈藥數
場景命名為 Level01，建好後存檔
```

Kiro 會建立完整的場景結構，包含可操控的玩家角色、地形、掩體物件和 UI，按 Play 就能用 WASD 移動、滑鼠環顧。

**Step 2 — 加入敵人和射擊系統**

場景有了，但還不能打。需要敵人和射擊機制。

```
幫我加入射擊遊戲的核心玩法：
1. 射擊系統：滑鼠左鍵射擊，用 Raycast 偵測命中，命中時敵人閃紅
2. 3 個敵人（紅色膠囊體），會朝玩家緩慢移動
3. 敵人被射中 3 次後消滅，消滅時播放粒子特效
4. 計分系統：畫面左上角顯示分數，每消滅一個敵人 +100 分
5. 所有敵人消滅後顯示「YOU WIN」
```

Kiro 會生成射擊控制器、敵人 AI、血量系統、計分 UI 等腳本，並把它們掛到對應的物件上。按 Play 就是一個能玩的射擊小遊戲。

**Step 3 — 建立關卡配置系統**

遊戲能玩了，現在需要一個讓企劃能在 Inspector 裡調整的關卡配置。
...
遊戲做出來了，接下來對它進行全面體檢，找出問題並優化。

**Step 4 — 程式碼品質檢查**

遊戲能跑了，但程式碼寫得好不好？架構有沒有走歪？
...
> 💡 **這就是 Kiro Unity Power 的核心價值**：從一句話做出一款能玩的遊戲，到全面品質檢測、效能優化、跨平台驗證，整條流水線都在聊天框裡完成。你只需要描述意圖，Kiro 負責規劃執行。
