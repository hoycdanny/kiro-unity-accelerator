...
> **Unity 6.x 対応、実プロジェクトでの検証を優先**: Steering ガイドは Unity 6 の各アップデートリリースで変更された挙動をカバーしています。URP のデフォルトとなった Render Graph レンダリングパス、GPU Resident Drawer（Forward+/Deferred+ 向けの GPU インスタンシングによるドローコール削減）、一部バージョンの新規プロジェクトでデフォルトとなる「シーンのみ再読み込み（ドメインリロードなし）」の Play Mode 挙動、実験的な CoreCLR スクリプティングバックエンド、そして Unity が公式に表明している OculusXR パッケージから OpenXR への移行方針などが含まれます。本 Power は記憶上のバージョン番号だけで挙動を断定せず、MCP 経由で接続中のプロジェクトの実際の Unity バージョン、現在有効なレンダーパイプライン、レンダリングパス、Scripting Backend、インストール済みパッケージを確認した上でバージョン固有のアドバイスを適用します。
